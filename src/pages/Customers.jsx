            ...
            <div className="flex gap-1 bg-gray-100 rounded-xl p-1 overflow-x-auto">
              {[{ value: "active", label: "Ativos" }, { value: "inactive", label: "Inativos" }, { value: "all", label: "Todos" }].map(opt => (
                <button
                  ...
                  className={cn("px-3 sm:px-4 py-1.5 rounded-lg text-sm font-medium transition-all whitespace-nowrap", statusFilter === opt.value ? "bg-white text-branding-primary shadow-sm" : "text-gray-500 hover:text-gray-700")}
                >
                  {opt.label}
                </button>
              ))}
            </div>
            <div className="hidden sm:flex border border-gray-200 rounded-xl overflow-hidden flex-shrink-0">
          ...
          <div className="flex gap-2 sm:gap-3 mt-3">
            <div className="relative flex-1 min-w-0">
              ...
              <Input
                ...
                className="pl-9 sm:pl-10 rounded-xl border-gray-200 text-sm"
              />
            </div>
            <div className="flex sm:hidden border border-gray-200 rounded-xl overflow-hidden flex-shrink-0">
              <button onClick={() => setViewMode("grid")} className={cn("px-2.5 py-2 transition-colors", viewMode === "grid" ? "bg-branding-primary text-white" : "hover:bg-gray-50")}>
                <LayoutGrid className="w-4 h-4" />
              </button>
              <button onClick={() => setViewMode("list")} className={cn("px-2.5 py-2 transition-colors", viewMode === "list" ? "bg-branding-primary text-white" : "hover:bg-gray-50")}>
                <List className="w-4 h-4" />
              </button>
            </div>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {filteredCustomers.map(customer => (
              <div key={customer.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6 hover:shadow-md transition-all">
                <div className="flex items-start justify-between mb-3 sm:mb-4">
                  <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                        <p className="text-sm text-gray-500">{getPlanName(customer)}</p>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem onClick={() => { setDeletingCustomer(customer); setConfirmAction("delete"); }} className="text-red-600">
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    <Phone className="w-4 h-4 text-gray-500" />{customer.whatsapp || "-"}
                    </div>
                    ...
                    <Badge className={cn("font-semibold", (customer.current_credits || 0) <= 1 ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700")}>
                ...
                <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-100 flex items-center justify-between">
                  <Badge variant="outline" className={cn(customer.status === "active" ? "border-emerald-300 text-emerald-700" : customer.status === "inactive" ? "border-gray-300 text-gray-500" : "border-amber-300 text-amber-700")}>
            ...
            <table className="w-full text-sm min-w-[500px]">
              <thead className="bg-gray-50/50 border-b border-gray-100">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 font-medium text-gray-600 hidden sm:table-cell">Plano</th>
                  {showCompanyColumn && <th className="text-left px-4 py-3 font-medium text-gray-600">Salão</th>}
                  <th className="text-left px-4 py-3 font-medium text-gray-600 hidden md:table-cell">WhatsApp</th>
                  <th className="text-left px-4 py-3 font-medium text-gray-600">Créditos</th>
                  <th className="text-left px-4 py-3 font-medium text-gray-600 hidden md:table-cell">Agendas</th>
                  ...
                  <tr key={customer.id} className="hover:bg-gray-50/50">
                    <td className="px-4 py-3">
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <span className="text-gray-600">{getPlanName(customer)}</span>
                        {customer.custom_plan && (
                          <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-purple-100 text-purple-700 font-medium">Personalizado</span>
                        )}
                        {customer.guardian_id && (
                    ...
                    {showCompanyColumn && <td className="px-4 py-3 text-gray-600 text-xs">
                      {(() => {
                        const compIds = customer.company_ids?.length ? customer.company_ids : (customer.company_id ? [customer.company_id] : (customer.companyId ? [customer.companyId] : []));
                    ...
                    <td className="px-4 py-3 text-gray-600 hidden md:table-cell">{customer.whatsapp || "-"}</td>
                    <td className="px-4 py-3">
                      <Badge className={cn("font-semibold", (customer.current_credits || 0) <= 1 ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700")}>
                    ...
                    <td className="px-4 py-3 hidden md:table-cell">
                      {(() => {
                        const stats = appointmentStats[customer.id];
                        if (!stats) return <span className="text-gray-500 text-xs">-</span>;
                        return (
                          <div className="text-xs space-y-0.5">
                            <span className="text-gray-600">{stats.total} serviços ({stats.present}P / {stats.absent}F)</span>
                            {stats.nextAppointment && (
