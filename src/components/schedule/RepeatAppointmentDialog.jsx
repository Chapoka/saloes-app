      ...
      <DialogContent className="sm:max-w-sm rounded-2xl">
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-2">
            <Label className="text-sm font-medium text-on-surface">Por quantas semanas?</Label>
          ...
          {previewDates.length > 0 && (
            <div className="text-xs text-on-surface-variant">
              ...
              {previewDates.map((d, i) => (
                <span key={i} className="inline-block mr-2 bg-surface-container px-2 py-0.5 rounded">{d}</span>
              ))}
              {parseInt(weeks) > 4 && <span className="text-on-surface-variant">+ mais...</span>}
            </div>
          )}
