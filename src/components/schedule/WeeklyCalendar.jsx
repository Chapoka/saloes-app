...
export default function WeeklyCalendar({ appointments, customers = [], onAppointmentClick, onSlotClick }) {
            ...
            {weekDays.map((day, i) => (
              ...
              >
                ...
                <p className={cn(
                  ...
                  isSameDay(day, new Date()) ? "text-branding-primary" : "text-gray-900"
                )}>
                  {format(day, "d")}
                </p>
              </div>
          ...
          <div className="relative grid grid-cols-8" style={{ height: timeSlots.length * 48 }}>
            ...
            <div className="border-r border-gray-100">
              {timeSlots.map((time, i) => (
                <div 
                  key={time} 
                  className="h-12 px-3 flex items-start justify-end pt-1 text-xs text-gray-500 border-b border-gray-50"
                >
                  {i % 2 === 0 && time}
                </div>
              ))}
              ...
              return (
                ...
                >
                  ...
                  {timeSlots.map((time, i) => (
                    <div
                      key={i}
                      className="h-12 border-b border-gray-50 cursor-pointer hover:bg-branding-primary/5 transition-colors"
                      onClick={(e) => {
                        e.stopPropagation();
                        ...
                        >
                          <div className="flex items-center justify-between gap-1 text-xs font-medium truncate">
                            ...
                            <div className="flex items-center gap-1 flex-shrink-0">
                              {appointment.isGroup && (
                                <span className="bg-white/60 text-[10px] px-1.5 py-0.5 rounded-full font-semibold">
