            ...
            {appointment.status !== "cancelled" && appointment.date && (
              <div className="p-3 bg-branding-primary/5 border border-branding-primary/10 rounded-xl space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <a
                    href={`https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(appointment.service_category === "corte" ? "Corte" : "Barba")}&dates=${appointment.date?.replace(/-/g, "")}T${(appointment.start_time || "09:00").replace(":", "")}00/${appointment.date?.replace(/-/g, "")}T${(appointment.end_time || "09:30").replace(":", "")}00`}
                    target="_blank"
                  ...
                  <a
                    href={`https://outlook.live.com/calendar/0/deeplink/compose?path=/calendar/action/compose&rru=addevent&subject=${encodeURIComponent(appointment.service_category === "corte" ? "Corte" : "Barba")}&startdt=${appointment.date}T${appointment.start_time || "09:00"}:00&enddt=${appointment.date}T${appointment.end_time || "09:30"}:00`}
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    <Button size="sm" variant="outline" className="h-7 text-xs border-outline-variant/30" type="button">
